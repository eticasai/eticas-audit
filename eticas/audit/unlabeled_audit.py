...
class UnlabeledAudit(BaseAudit):
    ...
    def run_audit(self, dataset_path : str,
        ...
        unlabeled_log = {}
        unlabeled_log.update({'da_inconsistency' : Da_inconsistency().compute(input_data, self.model.sensitive_attributes)})
        unlabeled_log.update({'da_positive' : Da_positive().compute(input_data, self.model.sensitive_attributes,
                                                    output_column, positive_output)})
        unlabeled_log.update({'dxa_inconsistency' : Dxa_inconsistency().compute(input_data, self.model.sensitive_attributes, 
                                                                              self.model.features)})
        unlabeled_log.update({'da_informative' : Da_informative().compute(input_data, self.model.sensitive_attributes,
                                                               output_column,self.model.features)})
        unlabeled_log.update({'d_statisticalparity' : D_statisticalparity().compute(input_data, self.model.sensitive_attributes,
                                                    output_column, positive_output)})
        unlabeled_log.update({'d_parity' : D_parity().compute(input_data, self.model.sensitive_attributes,
                                                    output_column, positive_output)})
