...
Implements an audit for the training phase of a model.
"""
...
class LabeledAudit(BaseAudit):
    """
    Audit for the labeled / training phase of a model.
    """

    def run_audit(self, dataset_path : str, label_column : str, 
                  output_column : str = None,positive_output : list = None):
        """
        Performs checks related to the labeled / training  dataset and process.
        ...
        logger.info(f"Running training audit on model '{self.model.model_name}'")
        ...
        labeled_log = {}
        labeled_log.update({'da_inconsistency' : Da_inconsistency().compute(input_data, self.model.sensitive_attributes)})
        labeled_log.update({'da_positive' : Da_positive().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, positive_output)})
        labeled_log.update({'dxa_inconsistency' : Dxa_inconsistency().compute(input_data, self.model.sensitive_attributes, 
                                                                              self.model.features)})
        labeled_log.update({'da_informative' : Da_informative().compute(input_data, self.model.sensitive_attributes,
                                                               label_column,self.model.features)})
        labeled_log.update({'d_statisticalparity' : D_statisticalparity().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, positive_output)})
        labeled_log.update({'d_parity' : D_parity().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, positive_output)})
        labeled_log.update({'poor_performance' : Performance().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, output_column)})
        
        labeled_log.update({'d_equalodds' : D_equalodds().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, positive_output, output_column)})
       
        labeled_log.update({'d_calibrated' : D_calibrated().compute(input_data, self.model.sensitive_attributes,
                                                    label_column, positive_output, output_column)})
        return labeled_log
