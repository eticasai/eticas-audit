...
class Tdx_inconsistency(BaseFairnessMetric):
    ...
    def compute(self, sensitive_attrs = None, input_features = None,
                input_data_dev : str = None, output_column_dev : str = None, positive_output_dev : list = [1],
                input_data_prod : str = None, output_column_prod : str = None, positive_output_prod : list = [1]):
        """
        Parameters
        ----------
        ...
        :param input_data: Dataset which should contain the sensitive_attrs and input_features.
        ...
        if output_column_dev is None:
            raise ValueError("You must provide output_column_dev to compute "+self.__str__())
        if positive_output_dev is None:
            raise ValueError("You must provide positive_output_dev to compute "+self.__str__())
        if output_column_prod is None:
            raise ValueError("You must provide output_column_prod to compute "+self.__str__())
        if positive_output_prod is None:
            raise ValueError("You must provide positive_output_prod to compute "+self.__str__())
        ...
        proportion_data = np.max([input_data_dev.shape[0],input_data_prod.shape[0]]) / np.sum([input_data_dev.shape[0],input_data_prod.shape[0]])

        json_groups = sensitive_attrs

        # this should be extract from schema.
        sensitive_columns = self.extract_sensitive_columns(json_groups)
        sensitive_columns = [col for col in sensitive_columns if (col in input_data_dev.columns) & (col in input_data_prod.columns) ]

        train_columns_0 = [c for c in input_data_dev.columns if c in input_features]
        train_columns_1 = [c for c in input_data_prod.columns if c in input_features]

        train_columns = [c for c in train_columns_0 if c in train_columns_1]
        ...
        if len(feat_columns) == 0 :
            raise ValueError('Total length of train and sensitive features is 0.')
        
        data = pd.concat([input_data_dev[feat_columns + [column_output]],input_data_prod[feat_columns + [column_output]]])
        # Split data (70% train, 30% test) based on the sensitive attribute
        train_data, test_data = train_test_split(data, test_size=0.3, 
                                                    stratify=data[column_output], random_state=123)
        ...
        for item in json_groups.items():
            ...
            try:
                ...
                data = pd.concat([input_data_dev[mask_underprivileged][feat_columns + [column_output]],input_data_prod[mask_underprivileged_2][feat_columns + [column_output]]])
                # Split data (70% train, 30% test) based on the sensitive attribute
                train_data, test_data = train_test_split(data, test_size=0.3, 
                                                            stratify=data[column_output], random_state=123)
                logistic_base = LogisticRegression(random_state=123)
